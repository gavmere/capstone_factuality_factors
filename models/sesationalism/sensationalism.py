...
class Sensationalism(FactualityFactor):
    ...
    def _prob_pos_from_out(self, out):
        ...
        for d in out:
            ...
            if lbl.startswith("LABEL_"):
                ...
                # custom string labels → invert id2label
        ...
        # Use pre-chosen positive id; fallback to max id
        if self.pos_id not in by_id:
            return float(by_id[max(by_id)])
        return float(by_id[self.pos_id])
    ...
    def _score_text(self, text: str, weight_headline: float = 0.6):
        ...
        # Score headline
        head_out = self.clf([headline])[0]
        head_prob = self._prob_pos_from_out(head_out)

        # Score body sentences (batched)
        if body_sents:
