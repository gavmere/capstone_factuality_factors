...
class EvaluationLogger:
    ...
    def start_run(
        ...
        self.current_log_path = os.path.join(
            self.logs_dir,
            f"evaluation_logs_{timestamp}.csv"
        )
    ...
    def log_result(
        ...
        row = {
            ...
            "gt_source_reputation": ground_truth.get("Source Reputation", ""),
            # LLM output
            ...
            "llm_source_reputation": llm_output.get("Source Reputation", ""),
            # Comparison
            "match_clickbait": comparison_results.get("Clickbait", False),
            "match_headline_body_relation": comparison_results.get("Headline-Body-Relation", False),
            "match_political_affiliation": comparison_results.get("Political Affiliation", False),
            "match_sensationalism": comparison_results.get("Sensationalism", False),
            "match_sentiment_analysis": comparison_results.get("Sentiment Analysis", False),
            "match_source_reputation": comparison_results.get("Source Reputation", False),
            # Error info
    ...
    def end_run(self, summary: Optional[Dict[str, Any]] = None):
        ...
        if self.current_run_id and summary:
            summary_path = os.path.join(
                self.metadata_dir,
                f"summary_{self.current_run_id}.json"
            )
    ...
    def list_runs(self) -> List[Dict[str, Any]]:
        ...
        if os.path.exists(self.logs_dir):
            for filename in os.listdir(self.logs_dir):
                if filename.startswith("evaluation_logs_") and filename.endswith(".csv"):
                    run_id = filename.replace("evaluation_logs_", "").replace(".csv", "")
