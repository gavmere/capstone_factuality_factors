...
NUMERIC_FACTORS = ["Clickbait", "Headline-Body-Relation", "Sensationalism", "Source Reputation"]
CATEGORICAL_FACTORS = ["Political Affiliation", "Sentiment Analysis"]

CATEGORICAL_VALUES = {
    "Political Affiliation": ["Democratic", "Democrat", "Republican", "Neutral", "Other"],
...
def normalize_column_name(col: str) -> str:
    ...
    if 'clickbait' in col_lower:
        return 'Clickbait'
    if ('political' in col_lower or 'party' in col_lower) and 'affiliation' in col_lower:
        return 'Political Affiliation'
    if 'sensationalism' in col_lower:
        return 'Sensationalism'
    if 'sentiment' in col_lower:
        return 'Sentiment Analysis'
    if 'source' in col_lower and 'reputation' in col_lower:
...
def validate_categorical_factor(value: Any, factor_name: str) -> Tuple[bool, str]:
    ...
    for valid in valid_values:
        if valid.lower() == value_lower or valid.lower() in value_lower or value_lower in valid.lower():
            return True, ""
    
    # Check for common variations
    if factor_name == "Political Affiliation":
        if any(term in value_lower for term in ['democrat', 'republican', 'neutral', 'other']):
            return True, ""
    elif factor_name == "Sentiment Analysis":
        if any(term in value_lower for term in ['positive', 'negative', 'neutral']):
            return True, ""
    
    return False, f"Invalid value '{value_str}' for {factor_name}. Expected one of: {valid_values}"
...
def validate_csv(file_path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        info.append(f"✅ Successfully loaded CSV with {len(df)} rows and {len(df.columns)} columns")
    except Exception as e:
        try:
            df = pd.read_csv(file_path, encoding='latin-1', quotechar='"', on_bad_lines='skip')
    ...
    has_url = any('url' in norm.lower() or 'article' in norm.lower() for norm in normalized_cols.values())
    has_content = any('content' in norm.lower() or 'body' in norm.lower() for norm in normalized_cols.values())
    ...
    for col in df.columns:
        ...
        elif 'headline' in col.lower() and 'body' in col.lower() and 'relation' in col.lower():
            found_factors.append((col, 'Headline-Body-Relation'))
    
    if not found_factors:
        issues.append("No factor columns found. Expected: Clickbait, Political Affiliation, Sensationalism, Sentiment Analysis, Source Reputation, Headline-Body-Relation")
    else:
        info.append(f"✅ Found {len(found_factors)} factor column(s): {[f[1] for f in found_factors]}")
    ...
    for orig_col, norm_col in found_factors:
        if norm_col in NUMERIC_FACTORS:
            ...
            if invalid_count > 0:
                issues.append(f"Column '{orig_col}' ({norm_col}): {invalid_count} invalid numeric values")
                issues.extend(invalid_examples[:5])
                if invalid_count > 5:
                    issues.append(f"  ... and {invalid_count - 5} more")
            else:
                info.append(f"✅ Column '{orig_col}' ({norm_col}): All numeric values valid (0-100)")
            ...
            if invalid_count > 0:
                issues.append(f"Column '{orig_col}' ({norm_col}): {invalid_count} invalid categorical values")
                issues.extend(invalid_examples[:5])
                if invalid_count > 5:
                    issues.append(f"  ... and {invalid_count - 5} more")
            else:
                info.append(f"✅ Column '{orig_col}' ({norm_col}): All categorical values valid")
                info.append(f"   Unique values found: {', '.join(map(str, unique_values[:10]))}")
...
def main():
    ...
    print(f"   Factor columns found: {', '.join(result['found_factors']) if result['found_factors'] else 'None'}")
