...
def calculate_metrics(
    ...
    if is_numeric:
        ...
        valid_pairs = [(gt, pred) for gt, pred in zip(gt_numeric, pred_numeric)
        ...
        gt_normalized = [normalize_categorical_value(str(gt), factor) for gt in ground_truth]
        pred_normalized = [normalize_categorical_value(str(pred), factor) for pred in predictions]
        
        # Filter out None/NaN values
        valid_pairs = [(gt, pred) for gt, pred in zip(gt_normalized, pred_normalized)
        ...
        if valid_pairs:
            ...
            try:
                ...
                precision, recall, f1, support = precision_recall_fscore_support(
                    gt_vals, pred_vals, labels=all_labels, zero_division=0, average='weighted'
                )
