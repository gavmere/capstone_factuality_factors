...
DEFAULT_SYSTEM_PROMPT = """You are a helpful assistant that analyzes news articles and provides scores for the following factors: Clickbait, Headline-Body-Relation, Party Affliation, Sensationalism, Sentiment Analysis, Toxicity.
...
DEFAULT_USER_PROMPT = "Analyze the following article and provide scores for the following factors: Clickbait, Headline-Body-Relation, Party Affliation, Sensationalism, Sentiment Analysis, Toxicity."
...
ALL_MODELS = {"Gemini": GEMINI_MODELS, "OpenRouter": OPENROUTER_MODELS}

# Initialize logger
logger = EvaluationLogger()

st.set_page_config(
    page_title="LLM Evaluation Testing Harness", page_icon="📊", layout="wide"
)

st.title("📊 LLM Evaluation Testing Harness")

# Sidebar for navigation
page = st.sidebar.selectbox("Navigation", ["Run Evaluation", "View Logs", "Past Runs"])
...
if page == "Run Evaluation":
    ...
    if df is not None:
        ...
        with col2:
            if provider == "Gemini":
                ...
                model_help = (
                    "Enter a Gemini model name (e.g., gemini-2.5-pro, gemini-1.5-pro)"
                )
            else:
            ...
            with st.expander("💡 Suggested Models"):
                if provider == "Gemini":
                    ...
                    for m in [
                        ...
                        if "openai" not in m and "anthropic" not in m
                    ]:
                        st.code(m, language=None)
        ...
        if st.button("🚀 Start Evaluation", type="primary", use_container_width=True):
            if not api_key:
                ...
                evaluator = Evaluator(
                    logger=logger, tolerance=tolerance, max_workers=max_workers
                )
                ...
                def progress_callback(completed, total):
                    ...
                    status_text.text(
                        f"Processed {completed}/{total} articles ({progress * 100:.1f}%)"
                    )
        ...
        if "last_results" in st.session_state:
                ...
                metrics_data.append(
                    {
                        ...
                        "MAE": f"{factor_metrics.get('mae', 'N/A')}"
                        if factor in NUMERIC_FACTORS
                        else "N/A",
                        "RMSE": f"{factor_metrics.get('rmse', 'N/A')}"
                        if factor in NUMERIC_FACTORS
                        else "N/A",
                        "F1": f"{factor_metrics.get('f1', 'N/A')}"
            ...
            for factor in CATEGORICAL_FACTORS:
                ...
                if cm and labels:
                    ...
                    if PLOTLY_AVAILABLE:
                        ...
                        st.info(
                            "💡 Install plotly (pip install plotly) to see visual confusion matrices"
                        )
            ...
            st.write(
                "Click on each article to view the LLM's raw response, ground truth, and parsed output."
            )

            # Create expandable sections for each article's LLM response
            for idx, result in enumerate(results["results"][:50]):  # Show first 50
                headline_preview = result.get(
                    "headline", f"Article {result['article_index']}"
                )
                ...
                with st.expander(
                    f"Article {result['article_index']}: {headline_preview}{error_indicator}"
                ):
                    col1, col2 = st.columns(2)
    ...
    if master_log is not None and len(master_log) > 0:
        ...
        with col1:
            ...
            selected_run = st.selectbox("Filter by Run ID", ["All"] + run_ids)
